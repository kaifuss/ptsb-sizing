...
Также для каждого источника запрашивается время на сканирование 1 файла в виртуальной машшине `time_to_scan`. И количество образов, которыми будет сканироваться каждый файл с данного источника `iso_amount`
...
```bash
dynamic_load = math.ceil(static_load * iso_amount * dynamic_cutoff * prefilter_cutoff * cache_cutoff)
```
...
```bash
dynamic_load = math.ceil(dynamic_load * iso_amount  * prefilter_cutoff * cache_cutoff)
```
...
| iso_amount             | Количество образов, которыми будет сканироваться каждый файл с данного источника, попадающий на проверку в виртуальные машины                | 1                     |
...
Введите количество образов ВМ, на которых будет сканироваться каждый файл с этого источника (по умолчанию - 1): 
```
