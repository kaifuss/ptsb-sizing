...
Зависимости приведены в файле *requirements.txt* в папке с утилитой и могут быть установлены при помощи следующей команды:<br>
```bash
...
Для каждого из вышеперечисленных вариантов поддерживается возможность выбора `типа инсталляции`. Также, для первых двух вариантов поддерживается ручной ввод `количества серверов` и соответствующего им `количества виртуальных машин` на сервер.
...
```
В случае согласия пользователя, т.е. ввода `yes`, `y`, `да` или `д`, скрипт начнет запрашивать параметры, по которым может быть рассчитана статическая и динамическая награузка на каждый из источников.
<br>Для каждого источника у пользователя запрашиваются коэффициенты отсечки:
...
Также для каждого источника запрашивается время на сканирование 1 файла в виртуальной машшине - `time_to_scan`. 
В итоге, с каждого источника вычисляется потенциальная динамическая нагрузка `dynamic_load` с округлением вверх как:
```bash
dynamic_load = math.ceil(static_load * dynamic_cutoff * prefilter_cutoff * cache_cutoff)
```
На основе полученной динамической нагрузки в час с данного источника высчиляется необходимое количество виртуальных машин `vms_needed` с округлением вверх как:
```bash
vms_needed = math.ceil(dynamic_load * time_to_scan / 3600)
```
Данные вычисления происходят для каждого из возможных источников. Итоговое `количество виртуальных машин` на всю инсталляцию вычисляется как сумма количества виртуальных машин с каждого из источников. Скрипт сравнивает полученное значение с максимальным возможным количеством виртуальных машин на 1 сервер, разрешенным вендором. Если полученное значение больше разрешенного, скрипт выдаст предупреждение об использовании инсталляции типа `AiO`:
```bash
...
```
Далее пользователю будет предложено ввести либо готовое `количество виртуальных машин` на всю инсталляцию, либо время сканирования одного файла `time_to_scan` в секундах:
```bash
...
```
В случае выбора варианта `2` скрипт запросит ввести это время `time_to_scan` в секундах и вычислит необходимое количество виртуальных машин на всю инсталляциию по формуле `vms_all = overall_dynamic * time_to_scan / 3600`:
```bash
──────────────────────────Количество ВМ на стенде───────────────────────────
Доступные варианты выбора количества виртуальных машин на инсталляцию:
1. Ввести общее количество ВМ на всю инсталляцию вручную
...
```
Аналогично предыдущему пункту, здесь таже выводится информация о `возможности использования инсталляции типа AiO`. 

---
#### 2.3 Выбор конфигурации и расчет параметров серверов
Переход к этому пункту происходит либо после получения показателей нагрузки `overall_static` и `overall_dynamic` на основании двух предыдущих пунктом, либо после выбора варианта расчета `3. Полностью ручный расчет ТХ серверов на основании вручную вводимого количества ВМ на сервер и количества серверов` при запуске скрипта.
От количества базовых образов `iso_amount`, которые будут использованы на стенде, зависит объем хранилища файлов `/opt/ptms/var/minio` на сервере(-ах) управления. Поэтому она запрашивается для определения дискового пространства:
```bash
...
1. Расчет ТХ под сервер управления с `vms_per_server` ВМ (максимально возможным числом, разшененным вендором)
2. Расчет ТХ под сервер управления с `vms_on_master` ВМ (тогда на каждом доп. сервере с динамикой будет равное максимально количество ВМ, разрешенным вендором)
...
```
Также количество дополнительных серверов и количество виртуальных машин для каждого может быть введено вручную:
```bash
...
1. Вопросы типа `yes`/`no`. В таких вопросах ввод `yes`, `y`, `да` или `да` означает согласие на действие; ввод `no`, `n`, `нет` или `н` означает отказ от действия. Нажатие `enter` в таком вопросе эквивалентно вводу `no`.
...
3. Вопросы типа *Выберите вариант*. В таких вопросах предполагает ввод одного целого числа от 1 до *N*, соответствующего любому из вариантов.
...
Для всей инсталляции и всех типов источников заданы различные параметры по умолчанию, чтобы упростить работу со скриптом в рамках однотипных рассчетов. Значения выбраны на основании личных взглядов автора проекта и Вы можете быть с ними не согласны. Ниже будет приведена информация о всех параметрах по умолчанию, которые можно изменить в исходном коде скрипта. Все параметры перечислены в соответстующих им объектах класса `dict()` ради сиситематизации работы скрипта.

---
**Общие параметры инсталяяции** 
<br>Задаются в блоке `__main__` и перечислены в объекте `installation_parameters`
...
| mails_with_attachments | Примерный процент количества писем от общего количества писем, которые могут содержать в себе вложения                   | 10                    |
...
<br>Задаются в блоке `__main__` и перечислены в объекте `smtp_load_parameters`
| Параметр         | Описание                                                                                                                         | Значение по умолчанию |
|:----------------:|:--------------------------------------------------------------------------------------------------------------------------------:|:---------------------:|
...
<br>Задаются в блоке `__main__` и перечислены в объекте `icap_load_parameters`
| Параметр           | Описание                                                                                                                               | Значение по умолчанию |
|:------------------:|:--------------------------------------------------------------------------------------------------------------------------------------:|:---------------------:|
...
<br>Задаются в блоке `__main__` и перечислены в объекте `automated_api_load_parameters`
| Параметр         | Описание                                                                                                                         | Значение по умолчанию |
|:----------------:|:--------------------------------------------------------------------------------------------------------------------------------:|:---------------------:|
...
<br>Задаются в блоке `__main__` и перечислены в объекте `manual_api_load_parameters`
| Параметр       | Описание                                                                                                         | Значение по умолчанию |
|:--------------:|:--------------------------------------------------------------------------------------------------------------:|:---------------------:|
| dynamic_cutoff | Примерный % количества файлов от общего количества, которые подходят под параметры динамических исследований     | 50                    |
...
<br>Задаются в блоке `__main__` и перечислены в объекте `storage_load_parameters`
| Параметр        | Описание                                                                                                         | Значение по умолчанию |
|:---------------:|:--------------------------------------------------------------------------------------------------------------:|:---------------------:|
| dynamic_cutoff  | Примерный % количества файлов от общего количества, которые подходят под параметры динамических исследований     | 90                    |
...
3. Количество писем с вложениями: неизвестно, поэтому предполагается, что из общего количества писем в час, примерно 50% содержат вложения
...
5. Прочие параметры, такие как: отсечка по префильтрации, отсечка по кэшированию, время сканирования 1 файла - остаются равным по умолчанию
6. Количество базовых образов, которые будут использоваться на инсталяции: 14
7. Тип инсталляции: отказоустойчивый кластер, серверов управления - 3
---
#### 1.1 Расчет нагрузки на основании используемых источников
При запуске скрипа выведется вопрос о типе расчета, который нам необходим. Т.к. в данном случае производится расчет для **проверки файлов с почтового трафика**, необходимо использовать вариант `1`:
```bash
Доступные варианты расчета технических характеристик под сервера PT SB:
1. Расчет ТХ серверов (а также их количества) на основании известных или около известных показателей нагрузки с разлиичных источников
2. Расчет ТХ серверов (а также их количества) на основании вручную вводимых показателей нагрузки на статику и динамику в час
...
├────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┤
│ SSD, Гб                    │                                325 │                                325 │                                325 │ 455                                   │
├────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┤
│ HDD, Гб                    │                                174 │                                174 │                                174 │ Не требуется                          │
╘════════════════════════════╧════════════════════════════════════╧════════════════════════════════════╧════════════════════════════════════╧═══════════════════════════════════════╛
```
...
│ /opt                 │                                 90 │                                 90 │                                 90 │ 210                                   │
├──────────────────────┼────────────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┤
│ /opt/ptms/var/minio  │                                174 │                                174 │                                174 │ Не требуется                          │
├──────────────────────┼────────────────────────────────────┼────────────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┤
│ /home                │                                100 │                                100 │                                100 │ 100                                   │
╘══════════════════════╧════════════════════════════════════╧════════════════════════════════════╧════════════════════════════════════╧═══════════════════════════════════════╛
```
Дополнительно, все расчеты сохраняются в файл `config.txt` на случай, если таблицы получаются больше ширины окна консоли.
...
```
На основании введенных параметров производится расчет необходимого `количества ВМ = 10` на всю инсталляцию, а также `возможности использования AiO`:
```bash
...
```
Далее скрипт запросит информацию о количестве дополнительных серверов. Т.к. нам необходимо ввести количество вручную, выбираем вариант `2`. Далее для каждого сервера будет запрошена информация о `количестве ВМ` на сервер, вводим `12`, `13` и `14` соответственно:
```bash
────────────────Ручная конфигурация доп. серверов динамики──────────────────
...
├────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┤
│ SSD, Гб                    │                                325 │ 439                                   │ 455                                   │ 471                                   │
├────────────────────────────┼────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┤
│ HDD, Гб                    │                                 97 │ Не требуется                          │ Не требуется                          │ Не требуется                          │
╘════════════════════════════╧════════════════════════════════════╧═══════════════════════════════════════╧═══════════════════════════════════════╧═══════════════════════════════════════╛
```
А также таблица с разметкой дискового пространства в гигабайтах:
```bash
...
│ /opt                 │                                 90 │ 194                                   │ 210                                   │ 226                                   │
├──────────────────────┼────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┤
│ /opt/ptms/var/minio  │                                 97 │ Не требуется                          │ Не требуется                          │ Не требуется                          │
├──────────────────────┼────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┼───────────────────────────────────────┤
│ /home                │                                100 │ 100                                   │ 100                                   │ 100                                   │
╘══════════════════════╧════════════════════════════════════╧═══════════════════════════════════════╧═══════════════════════════════════════╧═══════════════════════════════════════╛
```
